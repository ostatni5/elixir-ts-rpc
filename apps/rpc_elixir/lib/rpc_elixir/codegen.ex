defmodule RpcElixir.Codegen do
  ...
  `generate/2` reads the router's `__procedures__/0` list. Three commands run
  it: the `:elixir_ts_rpc` compiler (`Mix.Tasks.Compile.ElixirTsRpc`) on each
  compile, `mix rpc.gen.ts` (`Mix.Tasks.Rpc.Gen.Ts`) once, and
  `mix rpc.gen.ts.watch` on save. See [Getting started](getting-started.md),
  step 5.
  """
  ...
  Options:
    ...
    * `:out` — output file path. When set, handler source links in the JSDoc
      become absolute `file://` URIs, clickable in an editor. Otherwise they
